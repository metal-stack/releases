...
class MetalControlPlaneDeployment(unittest.TestCase):
    def __init__(self, *args, **kwargs):
        ...
        self.driver = Driver(os.getenv("METALCTL_API_URL").replace(
            "https://", "http://"), os.getenv("METALCTL_TOKEN"), os.getenv("METALCTL_HMAC"))
        self.maxDiff = None
    ...
    def test_switches_have_registered(self):
        ...
        for s in res:
            self.assertIsNotNone(
                s.last_sync, "switch has not synced: %s" % s.name)
            self.assertIsNone(s.last_sync.error,
                              "switch has sync errors: %s" % s.name)
...
class MetalSwitchPlaneDeployment(common.TestinfraCommon):
    def __init__(self, *args, **kwargs):
        ...
        self.is_sonic = self.hosts[0].file(
            "/etc/sonic/sonic_version.yml").exists
...
class GardenerControlPlaneDeployment(unittest.TestCase):
    ...
    def test_deployment(self):
        ...
        for ns in ["garden", "istio-ingress", "istio-system"]:
            ...
            for i in res.items:
                self.assertIsNone(i.status.unavailable_replicas,
                                  f"deployment {i.metadata.name} in namespace {ns} has unavailable replicas")
    ...
    def test_stateful_sets(self):
        ...
        for ns in ["garden", "istio-ingress", "istio-system"]:
            ...
            for i in res.items:
                self.assertEqual(i.status.current_replicas, i.status.replicas,
                                 f"stateful set {i.metadata.name} in namespace {ns} has unready replicas")
